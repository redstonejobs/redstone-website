...
test("candidate documents support controlled multi-file batches", () => {
  ...
  assert.match(uploader, /name="files"/);
  assert.match(uploader, /rows\.length >= 10/);
  assert.match(uploader, /\+ Add Another Document/);
  assert.match(uploader, /Upload All Documents/);
