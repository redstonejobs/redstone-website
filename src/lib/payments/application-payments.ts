...
export async function createOrReuseApplicationPayment({
  ...
  if (error || !application) {
    logPaymentFailure("candidate_payable_lookup", {
      applicationId,
      candidateId,
      error,
    });
  ...
  if (insertError || !payment) {
    logPaymentFailure("payment_insert", {
      applicationId,
      candidateId,
      error: insertError,
    });
  ...
  const { data: pendingApplication, error: pendingApplicationError } = await admin
    .from("applications")
  ...
  if (pendingApplicationError || !pendingApplication) {
    logPaymentFailure("application_payment_pending_update", {
      applicationId,
      candidateId,
    ...
    await admin
      ...
      .update({
        status: "failed",
        result_description: "Application could not enter payment processing.",
        failed_at: new Date().toISOString(),
      })
      .eq("id", payment.id)
      .eq("status", "initiated");

    throw new Error("Application could not enter payment processing.");
  }
...
export async function markApplicationReadyForPayment(
  ...
  if (error || !application) {
    logPaymentFailure("ready_for_payment_lookup", {
      applicationId,
      candidateId,
      error,
    });
  ...
  const { data: readinessRows } = await candidateClient
    .from("application_section_progress")
    .select("section_key, status")
    .eq("application_id", applicationId)
    .in("section_key", ["personal", "passport", "declarations", "documents"]);
  ...
  if (!["personal", "passport", "declarations", "documents"].every((key) => completed.has(key))) {
    throw new Error("Complete Personal, Passport, Declarations and Documents before payment.");
  }

  const privileged = createAdminClient();
  const { data, error: updateError } = await privileged
    .from("applications")
  ...
  if (updateError || !data) {
    logPaymentFailure("ready_for_payment_update", {
      applicationId,
      candidateId,
      error: updateError,
    });
...
export async function processMpesaCallbackPayload(payload: unknown) {
  ...
  if (String(payment.status) === "paid") {
    ...
    const paymentId = text(payment.id);
    const applicationId = text(payment.application_id);
    const candidateId = text(payment.candidate_id);

    if (!paymentId || !applicationId || !candidateId) {
      throw new Error("Paid payment record is incomplete.");
    }

    const { data: paidApplication, error: paidApplicationError } = await admin
      .from("applications")
      .select("status")
      .eq("id", applicationId)
      .eq("candidate_id", candidateId)
    ...
    if (paidApplicationError || !paidApplication) {
      throw new Error("Paid application could not be loaded.");
    }

    if (
      ["ready_for_payment", "payment_pending"].includes(
        String(paidApplication.status ?? "")
      )
    ) {
      const { error: finalizeError } = await admin.rpc(
        "submit_application_after_verified_payment",
        {
      ...
      if (finalizeError) {
        throw new Error("Paid application could not be finalized.");
      }
    }
    ...
    return {
      ok: true,
      status: 200,
      message: "Payment and application already finalized.",
    };
  }
  ...
  if (callback.resultCode === "0") {
    ...
    if (recordResult.shouldFinalize && recordResult.paymentId) {
      ...
      if (finalizeError) {
        throw new Error("Verified payment could not finalize the application.");
      }
    }
...
async function loadPayableApplication(
  ...
  const { data, error } = await supabase
    .from("applications")
    .select(
      "id, candidate_id, job_id, status, job:jobs(id, status, application_deadline, vacancies, employer:employers(verification_status, is_active))"
    )
    .eq("id", applicationId)
    .eq("candidate_id", candidateId)
    .maybeSingle<PaymentRow>();
  ...
  const employer = relation(job?.employer);
  const closed =
...
function logPaymentFailure(
  ...
  const error = supabaseError(details.error);

  console.error("[payments] application payment failed", {
    stage,
    application_id: details.applicationId ?? null,
    candidate_id: details.candidateId ?? null,
    payment_id: details.paymentId ?? null,
    code: error?.code ?? null,
    message: error?.message ?? errorMessage(details.error),
  });
}
