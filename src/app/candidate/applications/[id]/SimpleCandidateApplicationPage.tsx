...
export default async function SimpleCandidateApplicationPage({ params, searchParams }: Props) {
  const { id } = await params;
  const query = (await searchParams) ?? {};
  const requestedSection = typeof query.section === "string" ? query.section : "personal";
  const section = STEPS.find((step) => step.key === requestedSection)?.key ?? "personal";
  ...
  const fallbackFamilyName = nameParts.length > 1 ? nameParts[nameParts.length - 1] : "";
  const defaultGivenNames = profile.given_names ?? fallbackGivenNames;
  const defaultFamilyName = profile.family_name ?? fallbackFamilyName;
  const displayName =
    [profile.given_names, profile.family_name].filter(Boolean).join(" ") ||
    fullName ||
    "Not provided";
