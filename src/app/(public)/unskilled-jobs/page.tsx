...
export default async function UnskilledJobsPage() {
  const { jobs } = await getPublishedJobs({ skill: "unskilled", page: "1" });
  return (
    <>
      <Hero eyebrow="General labour" title="Unskilled and Entry-Level Opportunities" body="Unskilled does not mean no requirements. Employers may still require reliability, fitness, experience, checks, language ability or licenses." primary={{ label: "Browse All Jobs", href: "/jobs" }} />
      <Band tone="grey">
        <SectionHeading title="Currently Published Unskilled Jobs" />
