  ...
  return (
    <article className="flex h-full flex-col rounded-md border border-slate-200 bg-white p-5 shadow-sm transition hover:-translate-y-0.5 hover:shadow-md">
      <div className="flex-1">
        <div className="flex flex-wrap gap-2">
          {badges.map((badge) => (
            <span
              key={badge}
              className="rounded-full bg-[#F2D675]/45 px-3 py-1 text-xs font-black uppercase text-[#071A3D]"
            >
              {badge}
            </span>
          ))}
      ...
      <div className="mt-5 grid gap-2 sm:grid-cols-2">
        <Link
          href={jobHref(job)}
          className="rounded-md bg-[#071A3D] px-4 py-3 text-center text-sm font-black text-white"
        >
          View Details
        </Link>
        ...
        {external ? (
          externalApply ? (
            <a
              ...
              className="rounded-md border border-[#D4AF37] bg-[#D4AF37] px-4 py-3 text-center text-sm font-black text-[#071A3D]"
            >
              Apply at source
            </a>
          ) : (
            <span
              aria-disabled="true"
              className="pointer-events-none rounded-md border border-slate-200 bg-slate-100 px-4 py-3 text-center text-sm font-black text-slate-500"
            >
